...
class StatisticsController extends BaseController
{
	...
	{
		$serviceIdLeft = $this->getRequest()->getParam("serviceId");
	
		if ("" == $serviceIdLeft)
		{
			date_default_timezone_set('PRC');
			$date = date("Y-m-d H:i:s");
			$serviceIdLeft = substr($date,0,4);
		}
	
		$where = "LEFT(serviceId,4) = \"$serviceIdLeft\" ";
		...
		{
			array_push($serviceId,$value['serviceId']);
			...
			//查找对应的业务号下的各项是否超出阈值
			$temp = $value['serviceId'];
			$sql = "
					SELECT id FROM `costbill` WHERE `serviceId` = '$temp' ORDER BY id DESC LIMIT 1
					";
			...
			if (0 != $id) 
			{
				...
				if (sizeof($result) > 0)
				{
					//array_push($makeIsAlarm,$result[0]['makeIsAlarm']);
					//array_push($travelIsAlarm,$result[0]['travelIsAlarm']);
					
					if (1 == $result[0]['makeIsAlarm']) 
					{
						$temp = $make[$num];
						$make[$num] = "{'color':'red','y':$temp}";
					}
					if (1 == $result[0]['travelIsAlarm'])
					{
						$temp = $travel[$num];
						$travel[$num] = "{'color':'red','y':$temp}";
					}
				}
				else
				{
					//array_push($makeIsAlarm,0);
					//array_push($travelIsAlarm,0);
				}
			}
			else
			{
			...
			//查看餐费是否超出阈值
			$sql = "
			SELECT id FROM `mealsbill` WHERE `serviceId` = '$temp' ORDER BY id DESC LIMIT 1
			";
			$result = $db->query($sql)->fetchAll();
			...
			if (0 != $id)
			{
				...
				if (sizeof($result) > 0)
				{
					//array_push($mealsIsAlarm,$result[0]['mealsIsAlarm']);
					if (1 == $result[0]['mealsIsAlarm'])
					{
						$temp = $meals[$num];
						$meals[$num] = "{'color':'red','y':$temp}";
					}
				}
				else
				{
					//array_push($mealsIsAlarm,0);
				}
			}
			else
			{
		...
		/*print_r($serviceId);
		print_r($makeIsAlarm);
		print_r($travelIsAlarm);
		print_r($mealsIsAlarm);
		...
		$this->view->serviceId = $serviceIdLeft;
